...
namespace Antennas
{
    ...
    {
        private const double k = Consts.pi2 / Consts.SpeedOfLight;
        ...
        public static double GetRadiatingImpedance(double Length, double f0)
        {
            ...
            Func<double, double> F = theta => Math.Cos(kl * Math.Cos(theta)) - coskl;
            Func<double, double, double> core = (f, theta) => theta.Equals(0) ? 1 : f * f / Math.Sin(theta);
