...
namespace BeamServiceTests
{
    ...
    public class DigitalAntennaArray2Tests
    {
        ...
        public void Matrix_Beamforming_With_LFM_Signal()
        {
            ...
            const double source_angle_thetta = 0;
            const double source_angle_phi = 0;
            var radio_scene = new RadioScene
            {
                { new SpaceAngle(source_angle_thetta, source_angle_phi, AngleType.Deg), s }
            };

            const double beam_angle_thetta = 0;
        ...
        public void Matrix_Beamforming_With_Two_Sources()
        {
            ...
            const double source1_angle_thetta = +3.5, source1_angle_phi = 0;
            const double source2_angle_thetta = -3.5, source2_angle_phi = 0;
            var radio_scene = new RadioScene
            {
                { new SpaceAngle(source1_angle_thetta, source1_angle_phi, AngleType.Deg), source2 },
                { new SpaceAngle(source2_angle_thetta, source2_angle_phi, AngleType.Deg), source1 },
            };

            const double beam1_angle_thetta = 3.5, beam1_angle_phi = 0;
            beam_forming.PhasingАngle = new SpaceAngle(beam1_angle_thetta, beam1_angle_phi, AngleType.Deg);

            var signal1 = antenna_array.GetSignal(radio_scene);

            const double beam2_angle_thetta = -3.5, beam2_angle_phi = 0;
            beam_forming.PhasingАngle = new SpaceAngle(beam2_angle_thetta, beam2_angle_phi, AngleType.Deg);

            var signal2 = antenna_array.GetSignal(radio_scene);

            //const double beam3_angle_thetta = 60, beam3_angle_phi = 60;
            //beam_forming.PhasingАngle = new SpaceAngle(beam3_angle_thetta, beam3_angle_phi, AngleType.Deg);
